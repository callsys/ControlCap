...
from reem.models.tag_heads.bert import BertConfig, BertLMHeadModel, BertModel
...
if version == "blip2":
    @registry.register_model("reem_t5")
    class ReemT5(Blip2T5):
        ...
        def from_config(cls, cfg):
            cfg["vit_model"] = cfg.get("vit_model", "eva_clip_g")
            cfg["img_size"] = cfg.get("image_size")
            cfg["num_query_token"] = cfg.get("num_query_token")
            cfg["t5_model"] = cfg.get("t5_model")

            cfg["drop_path_rate"] = cfg.get("drop_path_rate", 0)
            cfg["use_grad_checkpoint"] = cfg.get("use_grad_checkpoint", False)
            cfg["vit_precision"] = cfg.get("vit_precision", "fp16")
            cfg["freeze_vit"] = cfg.get("freeze_vit", True)

            cfg["prompt"] = cfg.get("prompt", "")
            cfg["max_txt_len"] = cfg.get("max_txt_len", 32)

            cfg["apply_lemmatizer"] = cfg.get("apply_lemmatizer", False)
            model = cls(**cfg)
            if cfg.pretrained is not None:
                model.load_checkpoint(url_or_filename=cfg.pretrained)
            return model

if version == "blip2_cve":
    @registry.register_model("reem_t5")
    class ReemT5(Blip2T5):
        def __init__(self, *args, **kwargs):
            self.kwargs = kwargs
        ...
        def forward(self, samples):
            ...
            with self.maybe_autocast(dtype=torch.float16):
                embeds = self.ln_vision(self.visual_encoder(image))
                embeds = self.cve_forward(samples, embeds)
                control_tags = [""] * len(embeds)
                crl_embeds, crl_tokens = self.cee_forward(control_tags, embeds)
...
if version == "controlcap_no_beb":
    ...
    class ReemT5(Blip2T5):
        def __init__(self, *args, **kwargs):
            ...
            # Trainable parameters
            names = ["cve", "cee", "tag", "Qformer", "t5_proj"]
            self.finetune_llm = kwargs.get("finetune_llm", False)
            if self.finetune_llm:
                lora_config = LoraConfig(
                    r=64, lora_alpha=128, lora_dropout=0.0,
...
if version == "controlcap":
    ...
    class ReemT5(Blip2T5):
        def __init__(self, *args, **kwargs):
            ...
            # Contextual Object Embedding (COE) module
            self.roi_size = (16, 16)
            self._roi_align = torchvision.ops.RoIAlign(output_size=self.roi_size, spatial_scale=1 / 14,
                                                       sampling_ratio=2)

            self.coe_mlp = nn.Sequential(nn.Linear(2816, 1408), nn.ReLU(), nn.Linear(1408, 1408))

            # Controllable Object Embedding (CEE) module
            self.cee_memory = nn.Parameter(torch.zeros(self.llm_dim))

            # beb module
            beb_dim = 128
            self.beb_c2l_mlp = nn.Linear(self.llm_dim, beb_dim)
            self.beb_l2c_mlp = nn.Linear(beb_dim, self.llm_dim)
            self.beb_o2l_mlp = nn.Linear(1408, beb_dim)
            self.beb_l2o_mlp = nn.Linear(beb_dim, 1408)
            ...
            # Trainable parameters
            names = ["coe", "cee", "beb", "Qformer", "t5_proj"]
            self.finetune_llm = kwargs.get("finetune_llm", False)
            if self.finetune_llm:
                lora_config = LoraConfig(
                    r=64, lora_alpha=128, lora_dropout=0.0,
        ...
        def param_ratio(self, name):
            name_params = 0
            all_params = 0
            for param_name, param in self.named_parameters():
                all_params += param.numel()
                if name in param_name:
                    name_params += param.numel()
            return name_params/all_params

        def prepare_key_words(self, samples, drop_ratio=0.8, with_key_words=False, ans_key_words=False):
            key_words = []
            ...
            if self.training:
                for ans in answers:
                    try:
                        s2 = TextBlob(ans).tags
                        tokens = [el[0] for el in s2]
                        nouns = [name for name, value in s2 if "NN" in value]
                        if len(nouns) > 0:
                            words = []
                            for noun in nouns:
                                st_idx = tokens.index(noun)
                                ed_idx = st_idx + 1
                                while (ed_idx < len(tokens)) and (tokens[ed_idx] in nouns):
                                    ed_idx = ed_idx + 1
                                word = " ".join(tokens[st_idx:ed_idx])
                ...
                key_words = [random.choice(words) for words in key_words]
                key_words = [word if random.uniform(0, 1)>drop_ratio else "" for word in key_words]
            else:
                if ans_key_words:
                    key_words = answers
                else:
                    for ans in answers:
                        ...
                        key_words.append(words)
                    key_words = [words[0] for words in key_words]

                if not with_key_words:
        ...
        def coe_forward(self, samples, embeds):
            ...
            rois_embeds = self.roi_align(image_embeds, samples)
            object_embeds = torch.cat([rois_embeds, region_embeds], -1)
            object_embeds = self.coe_mlp(object_embeds)

            return object_embeds

        def cee_forward(self, samples, embeds):
            key_words = self.prepare_key_words(samples)
            ...
            return c_embeds + self.cee_memory, c_tokens, key_words
        ...
        def loss(self, inputs_embeds, encoder_atts, samples):
            answers = samples["answers"]
        ...
        def forward(self, samples):
            ...
            with self.maybe_autocast():
                embeds = self.ln_vision(self.visual_encoder(image))
                o_embeds = self.coe_forward(samples, embeds)
                c_embeds, c_tokens, key_words = self.cee_forward(samples, embeds)
                o_embeds, c_embeds, _ = self.beb_forward(o_embeds, c_embeds)

            with self.maybe_autocast(dtype=torch.bfloat16):
                object_atts = torch.ones(o_embeds.size()[:-1], dtype=torch.long).to(
                    image.device
                )
                query_tokens = self.query_tokens.expand(o_embeds.shape[0], -1, -1)
                query_output = self.Qformer.bert(
                    query_embeds=query_tokens,
                    encoder_hidden_states=o_embeds,
                    encoder_attention_mask=object_atts,
                    return_dict=True,
                )
                inputs_t5 = self.t5_proj(query_output.last_hidden_state)
                ...
                return self.loss(inputs_embeds, encoder_atts, samples)

        def visualize_attn(self, samples, c_tokens, key_words, attns):
            attns_c2o, attns_o2c = attns
            region_images = samples["region_images"].detach()
            mean = torch.Tensor((0.48145466, 0.4578275, 0.40821073))[None, None]
            std = torch.Tensor((0.26862954, 0.26130258, 0.27577711))[None, None]
            input_ids = c_tokens.input_ids
            attention_mask = c_tokens.attention_mask[:, None]

            attns_c2o = (attns_c2o[:, 1:].softmax(-1) * attention_mask).sum(-1).cpu()
            attns_c2o = attns_c2o.reshape(-1, 16, 16).detach()

            attns_o2c = (attns_o2c.permute(0, 2, 1)[:, 1:] * attention_mask).softmax(-1).sum(-1).cpu()
            attns_o2c = attns_o2c.reshape(-1, 16, 16).detach()
            ...
            for id, image, key_word, attn_c2o, attn_o2c in zip(ids, region_images, key_words, attns_c2o, attns_o2c):
                image = image.permute(1, 2, 0).cpu()
                image = image * std + mean
                image = (image.numpy() * 255.).astype(np.uint8)[:, :, ::-1]
                attn_c2o = (attn_c2o - attn_c2o.min()) / (attn_c2o.max() - attn_c2o.min())
                cam_c2o = (attn_c2o.numpy() * 255.).astype(np.uint8)
                heatmap_c2o = cv2.applyColorMap(cv2.resize(cam_c2o, (224, 224)), cv2.COLORMAP_JET)

                attn_o2c = (attn_o2c - attn_o2c.min()) / (attn_o2c.max() - attn_o2c.min())
                cam_o2c = (attn_o2c.numpy() * 255.).astype(np.uint8)
                heatmap_o2c = cv2.applyColorMap(cv2.resize(cam_o2c, (224, 224)), cv2.COLORMAP_JET)

                # image = (image*std + mean).numpy(np.uint8)
                blend_c2o = image * 0.4 + heatmap_c2o * 0.6
                blend_c2o = blend_c2o.astype(np.uint8)

                blend_o2c = image * 0.6 + heatmap_o2c * 0.4
                blend_o2c = blend_o2c.astype(np.uint8)

                save_path = os.path.join(save_dir, f"{str(id)}_{key_word}_org.jpg")
                cv2.imwrite(save_path, image)

                save_path = os.path.join(save_dir, f"{str(id)}_{key_word}_c2o.jpg")
                cv2.imwrite(save_path, blend_c2o)

                save_path = os.path.join(save_dir, f"{str(id)}_{key_word}_o2c.jpg")
                cv2.imwrite(save_path, blend_o2c)
        ...
        def predict_answers(
            ...
            with self.maybe_autocast():
                ...
                o_embeds = self.coe_forward(samples, embeds)
                c_embeds, c_tokens, key_words = self.cee_forward(samples, embeds)
                o_embeds, c_embeds, attn = self.beb_forward(o_embeds, c_embeds)

                # self.visualize_attn(samples, c_tokens, key_words, attn)
            ...
            with self.maybe_autocast(dtype=torch.bfloat16):
                ...
                outputs = self.t5_model.generate(
                    prefix_allowed_tokens_fn=conditional_func,
                    inputs_embeds=inputs_embeds,
                    attention_mask=encoder_atts,
                    do_sample=False,
                    num_beams=num_beams,
        ...
        def from_config(cls, cfg):
            ...
            if len(cfg.pretrained) != 0:
                model.load_checkpoint(url_or_filename=cfg.pretrained)
            if len(cfg.finetuned) != 0:
                model.load_checkpoint(url_or_filename=cfg.finetuned)
            if len(checkpoint) !=0:
                model.load_checkpoint(url_or_filename=checkpoint)
...
if version == "reem":
    ...
    class ReemT5(Blip2T5):
        ...
        def prepare_control_tags(self, samples, tag_logits, drop_ratio=0.2, tag_thr=0.5):
            control_tags = []
...
if version == "reem2":
    ...
    class ReemT5(Blip2T5):
        ...
        def prepare_control_tags3(self, samples, tag_logits, full_drop_ratio=0.5, drop_ratio=0.5, tag_thr=(0.85, 0.8)):
            control_tags = []
        ...
        def prepare_control_tags4(self, samples, tag_logits, full_drop_ratio=0.7, drop_ratio=0.5, tag_thr=0.7):
            control_tags = []
...
if version == "debug":
    ...
    class ReemT5(Blip2T5):
        ...
        def prepare_control_tags(self, samples, tag_logits, full_drop_ratio=0.2, drop_ratio=0.5, tag_thr=(0.7, 0.7)):
            control_tags = []
        ...
        def prepare_control_tags_otags(self, samples, tag_logits, full_drop_ratio=0.7, drop_ratio=0.5, tag_thr=0.8):
            control_tags = []
